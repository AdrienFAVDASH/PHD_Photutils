...
def process_images(detection_image, filter_images, nsigma, npixels, nlevels=32, connectivity=8, contrast=0.0001, mode='exponential', smooth_data=True, kernel_name='Tophat', smooth_fwhm=2, kernel_size=5, progress_bar=True, show_plots=True, apermask_method='correct', kron_params=[1.1, 1.6], radius=3.0, columns=None, output_hdf5_filename='Photutils.hdf5'):
    """
    ...
    - columns : see to_table and to_dataframe
...
class Image:
    ...
    def initialize_photometry(self, detection_image, apermask_method='correct', kron_params=[1.1, 1.6]):
        ...
        """
        self.detection_cat = SourceCatalog(data=detection_image.sci, segment_img=detection_image.segm_deblend, convolved_data=detection_image.sci, error=detection_image.err, background=detection_image.bkg, apermask_method=apermask_method, kron_params=kron_params, progress_bar=True)
        self.photometry_cat = SourceCatalog(data=self.sci, segment_img=detection_image.segm_deblend, convolved_data=self.sci, error=self.err, background=self.bkg, apermask_method=apermask_method, kron_params=kron_params, detection_cat=self.detection_cat, progress_bar=True)
    ...
    def perform_kron_photometry(self, kron_params=[1.1, 1.6], name='kron_phot', overwrite=True):
        """
    ...
    def to_table(self, columns=None, save_to_file=True, filename='output_file.h5', format='hdf5'):
        ...
        Parameters :
        - columns : List of column names to include in the table. Default : None, however the circular aperture and kron photometry columns are added automatically and don't have to be provided.
